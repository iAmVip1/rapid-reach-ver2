...
import { FaPhone, FaTrash, FaEye, FaEdit, FaUser, FaEnvelope, FaBuilding, FaCircle, FaSearch, FaFilter, FaHospital, FaMapMarkerAlt, FaCalendar, FaCheck, FaTimes, FaFire } from 'react-icons/fa';
import { Link } from 'react-router-dom';

export default function DashFire() {
  ...
  useEffect(() => {
    const fetchFirePosts = async () => {
      try {
        setLoading(true);
        const res = await fetch('/api/post/admin/get?category=Fire%20Department&includeUnapproved=true', {
          credentials: 'include',
        });
        const data = await res.json();
        console.log('Fire Department API Response:', data); // Debug log
        
        if (res.ok) {
          // Handle the correct data structure: { success: true, count: number, data: array }
          const posts = data.data || [];
          setFirePosts(posts);
          console.log('Fire department posts found:', posts.length); // Debug log
        } else {
          console.log('Failed to fetch fire department posts:', data);
        }
      } catch (error) {
        console.log('Error fetching fire department posts:', error.message);
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const mockOnlineUsers = firePosts.slice(0, 2).map(post => post.userRef);
  ...
  const filteredPosts = firePosts.filter(post => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading fire department posts...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Fire Department Management</h1>
          <p className="text-gray-600">Manage all fire department posts and their services</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Total Fire Departments</p>
                <p className="text-2xl font-bold text-gray-900">{firePosts.length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Online Fire Departments</p>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{new Set(firePosts.map(post => post.category)).size}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Verified Fire Departments</p>
                <p className="text-2xl font-bold text-gray-900">{firePosts.filter(post => post.userMail).length}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100">
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Search fire departments by name or address..."
                value={searchTerm}
                  ...
                  <tr key={post._id} className="hover:bg-gray-50 transition-colors duration-200">
                      ...
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-12 w-12">
                          <img
                            ...
                            src={post.imageUrls?.[0] || 'https://via.placeholder.com/48x48?text=F'}
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center space-x-2">
                        ...
                        <button
                          onClick={() => handleCallFireDepartment(post)}
        ...
        {filteredPosts.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-800 mb-2">No fire department posts found</h3>
            <p className="text-gray-600">Try adjusting your search or filter criteria.</p>
            <div className="mt-4 text-sm text-gray-500">
              <p>Debug Info:</p>
              <p>Total posts in database: {firePosts.length}</p>
