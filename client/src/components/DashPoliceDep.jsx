...
export default function DashPoliceDep() {
  ...
  useEffect(() => {
    const fetchPolicePosts = async () => {
      try {
        setLoading(true);
        const res = await fetch('/api/post/admin/get?category=Police%20Department&includeUnapproved=true', {
          credentials: 'include',
        });
        const data = await res.json();
        if (res.ok) {
          const posts = data.data || [];
          setPolicePosts(posts);
        } else {
          console.log('Failed to fetch police posts:', data);
        }
      } catch (error) {
        console.log('Error fetching police posts:', error.message);
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredPosts = policePosts.filter(post => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Loading police department posts...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Police Department Management</h1>
          <p className="text-gray-600">Manage all police department posts and their services</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex items-center">
              <div className="p-3 bg-emerald-100 rounded-lg">
                <FaBuilding className="w-6 h-6 text-emerald-600" />
              </div>
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Total Units</p>
                <p className="text-2xl font-bold text-gray-900">{policePosts.length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">Online Units</p>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{policePosts.filter(post => post.userMail).length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{new Set(policePosts.map(post => post.category)).size}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100">
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Search police units by name or address..."
                value={searchTerm}
                      ...
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-12 w-12">
                          <img
                            ...
                            src={post.imageUrls?.[0] || 'https://via.placeholder.com/48x48?text=P'}
        ...
        {filteredPosts.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-800 mb-2">No police department posts found</h3>
