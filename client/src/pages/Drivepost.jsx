...
export default function Drivepost() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // basic validations
      if (!licenseImage) return setError('You must select a license image');
      if (
        vehicleDocs.length === 0 &&
        (!Array.isArray(formData.documentUrls) ||
          formData.documentUrls.length === 0)
      ) {
        return setError('You must attach at least one vehicle document');
      }
      ...
      if (licenseImage) {
        // Multer expects single 'image' field
