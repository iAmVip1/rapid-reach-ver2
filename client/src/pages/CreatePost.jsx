...
export default function CreatePost() {
  ...
  // (Multer handles upload on submit; no separate Cloudinary image upload here)
  ...
  const handleUploadDocument = (e) => {
    e.preventDefault();
    alert("Document uploads are handled separately or on a different flow.");
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      if (!image) return setError("You must select an image file");
      if (documents.length === 0 && formData.documentUrls.length === 0)
        return setError("You must select at least one supporting document");
      ...
      // Attach single image file for Multer (field name: 'image')
